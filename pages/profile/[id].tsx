    ...
    <div className="w-full">
      <div className="flex gap-6 md:gap-10 mb-4 bg-white w-full">
        ...
        <div className="flex flex-col justify-center">
          ...
          <p className="capitalize md:text-xl text-gray-400 items-center text-xs">
            {user.userName}
          </p>
        </div>
      </div>
      ...
      <div>
        ...
        <div className="flex gap-6 flex-wrap md:justify-start">
          {videoList.length > 0 ? (
            videoList.map((post: Video, idx: number) => <VideoCard post={post} key={idx} />)
          ) : (
            <NoResults
              text={`No ${showUserVideos ? '' : 'liked'}
          Video Yet`}
            />
          )}
        </div>
